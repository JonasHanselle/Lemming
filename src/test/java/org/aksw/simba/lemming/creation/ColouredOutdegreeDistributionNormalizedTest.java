...
public class ColouredOutdegreeDistributionNormalizedTest {

	private static final double DELTA = 0.01;

	/*
	 * Test for FeatureGraphGenerator for ColouredOutdegreeDistributions based on the
	...
	public void test() {
		...
		ColouredGraph cGraph = g.generateGraphColouredOutDegreeNormalized(expectedDistribution, edgeColourDistribution, numberNodes);
		...
		for (BitSet c : expectedDistribution.keySet()) {
			...
			for (int i = 0; i < expectedValues.length; ++i) {
				Assert.assertEquals("Error for colour " + c.toString(), expectedValues[i], distribution.values[i]/numberNodes,
						DELTA);
			}
